...
- Optional built-in oscilloscope with 2 switchable probe inputs (BOOT button or double-click to swap live), 0-3.3V RC signals natively, up to 5V per probe with its own external 16.2k/24.9k voltage divider. A short button press cycles the encoder between timebase, trigger level and per-probe voltage calibration (trim against a known reference voltage, saved to EEPROM per probe) - plus a 3-waveform (sine/triangle/rectangle) signal generator
