...
- Web "Arcade Mode": two big touch-drag controls for driving from a phone in landscape - vertically centered left (steering) and right (throttle), like holding a game controller - both spring back to center on release, driving the same 2 channels as the physical joystick
...
- **Web "Arcade Mode"**: a driving-style control page for phones, reachable from the web menu or a link on the Servo Tester page. A horizontal steering bar (left half, left/right) and a vertical throttle bar (right half, forward/reverse), both vertically centered on the screen - within natural thumb reach for a two-handed landscape grip, like holding a game controller. Drag with a thumb each, release to spring back to center. Drives the same two channels as the physical joystick (JOYSTICK_X_CHANNEL/JOYSTICK_Y_CHANNEL from Settings), over the same low-latency WebSocket channel the position sliders already use - it's really just an alternative input for the same feature, not a separate one.
