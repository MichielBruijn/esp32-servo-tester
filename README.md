...
- Joystick channel linking: extra channels can be driven in parallel with Steer or Throttle (e.g. two steering servos), each remapped to its own Min/Center/Max so it stays symmetric even with different calibration. Only applies in Joystick Mode - dragging the Steer/Throttle channel's own slider in Manual Mode moves just that channel, not its linked partners
