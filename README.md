...
- Joystick Steering Limit: an optional 0-100% setting that progressively reduces Steer deflection as Throttle moves away from center (forward or reverse), so full steering lock is available at parking speed but backed off under throttle
...
- **Boot screen**: just a brief neutral splash (no logo/branding) now - the control-help screen that used to follow it (dismissed on a button press or a 10s timeout) was dropped since the same content is available on demand on the Info page's Controls screen. WiFi SSID/password are no longer shown as a boot popup either, available on demand in the Settings menu instead.
...
- **Web "Joystick Mode"** (was "Arcade Mode"): a driving-style control page for phones, reachable from the web menu. A horizontal steering bar and a vertical throttle bar, side by side and vertically centered on the screen - within natural thumb reach for a two-handed landscape grip, like holding a game controller. Custom touch tracks (Pointer Events, `requestAnimationFrame`-batched, moved via CSS `transform`), not native `<input type="range">` - a native/custom mix left whichever control was touched *second* completely unresponsive on Android Firefox, so both ended up the same custom implementation. That in turn exposed a real Android Firefox quirk: with `e.preventDefault()` in the touch handlers, the *second* simultaneous touch's move-event rate was throttled roughly 14x (confirmed via a temporary on-screen diagnostic: steady 120fps, but ~8 events/sec instead of ~112) - removing `preventDefault()` (redundant anyway once `touch-action: none` is set) fixed it; both fingers now track independently and smoothly. Drag with a thumb each, release to spring back to center. Which channel each bar drives is configurable in Settings ("Joystick Steer"/"Joystick Throttle"), over the same low-latency WebSocket channel the position sliders already use.
- **Physical analog joystick removed**: this fork briefly had one (2-axis stick wired to GPIO34/35/26, with a deadzone, self-calibrating range, and a click button), replaced by the web-only Joystick Mode above - one less thing to wire up, and the channel-mapping settings ("Joystick Steer"/"Joystick Throttle" in Settings) carried over unchanged.
- **Menu reorganized**: the top-level list is now Manual Mode (was Servo Tester), Sweep Mode (was Automatic Mode), Expert Functions, Joystick Mode, Info, Settings. Expert Functions is a new nested submenu (its own turn-to-browse, press-to-enter list) grouping the less commonly used, more technical screens: Read PWM Impulse, Read PPM Multiswitch, Read SBUS, Read IBUS, Oscilloscope and Signal Generator - keeping the top-level list short while still making all of them reachable.
- **Joystick channel linking + Steering Limit**: Settings ("Joystick" group, web interface only for the linking checkboxes) now lets you drive extra channels in parallel with Steer or Throttle - each linked channel is remapped via its own Min/Center/Max, so e.g. two steering servos with different calibrated end points still move symmetrically. A "Steering Limit" setting (0-100%, OLED and web) progressively reduces Steer deflection as Throttle deflection increases in either direction, re-applied continuously so it also reacts to a Throttle-only change without touching Steer.
- **Factory Reset confirmation on the OLED**: setting it to "Yes" and confirming now shows a dedicated "Factory Reset?" screen (turn to change, short press to apply, long press to cancel) before anything is actually wiped - previously a single short press applied it immediately. The web interface already had a confirm dialog for this.
- **Consistent "Menu" button color** across every web page (was red almost everywhere, grey on the Settings page specifically).
- **Firmware update via GitHub Releases**: in WiFi Station mode with internet access, the device periodically checks `api.github.com` for this repo's latest release and compares its tag against the running `codeVersion`. If newer, an "Update available" notice appears on the OLED Info page (firmware page, short press installs, with a live written/total percentage instead of a static screen) and on every web page (banner + Info page, "Install Update" button). Installing streams the release's `firmware.bin` asset straight into the OTA partition via ESP32's `Update` library and restarts on success; the running firmware is left untouched if the download or write fails. `GET /update/on` answers with an HTTP redirect rather than a normal page, so refreshing the browser afterwards can't silently replay the same request and re-trigger another install. The buzzer is explicitly silenced the moment an update starts, since its own off-timer only runs from the main loop, which the update blocks for its whole duration. Uses `WiFiClientSecure::setInsecure()` (no certificate pinning) since the ESP32 has no practical way to keep a GitHub TLS cert fresh - a deliberate trade-off for a hobby project checking its own public repo. Adds ~13% flash usage (mbedTLS/HTTPClient) - still comfortably inside the OTA partition, but with less headroom for future growth. Getting this reliable took three rounds of live-testing fixes: GitHub's release-asset redirect goes to a different host (objects.githubusercontent.com), which HTTPClient's own redirect-following reuses the old connection for (hangs) - fixed by resolving redirects manually with a genuinely fresh `WiFiClientSecure`/`HTTPClient` per hop (even `client.stop()`-and-reuse left stale socket state); and `WiFiClient::setTimeout()` takes seconds, not milliseconds, so an intended 15s read timeout was actually ~4.2 hours, letting any real network stall hang indefinitely.
- **Firmware download/upload with no internet needed**: the Info page's "Download Firmware" streams the currently-running image (byte-exact with what `pio run` produces) as a `.bin` file; "Upload Firmware" posts a chosen file straight to the OTA partition via `Update`, no GitHub or internet involved on either end - useful for copying a known-good build between devices, or where WiFi Station/internet isn't available at all.
