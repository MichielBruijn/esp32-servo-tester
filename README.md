...
- Web "Arcade Mode": two big touch-drag controls for driving from a phone in landscape - a horizontal steering bar and a vertical throttle bar, both spring back to center on release, driving the same 2 channels as the physical joystick
...
- **Web "Arcade Mode"**: a driving-style control page for phones, reachable from the web menu or a link on the Servo Tester page. A full-width horizontal bar (steering, left/right) and a vertical bar on the right (throttle, forward/reverse) - drag with a thumb each, release to spring back to center. Drives the same two channels as the physical joystick (JOYSTICK_X_CHANNEL/JOYSTICK_Y_CHANNEL from Settings), over the same low-latency WebSocket channel the position sliders already use - it's really just an alternative input for the same feature, not a separate one.
