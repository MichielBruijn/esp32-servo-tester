...
- Optional built-in oscilloscope with 2 switchable probe inputs (BOOT button or double-click to swap live), 0-3.3V RC signals natively, up to 5V per probe with its own external 16.2k/24.9k voltage divider, reliable up to roughly 500Hz at the default timebase (RC PWM/PPM territory - the ADC sampling rate is the ceiling, not a hard limit, so the fastest timebase setting can push usable frequency measurement into the low kHz range at the cost of accuracy). Trigger level auto-recenters on the signal every frame, so frequency/pulsewidth locks regardless of amplitude or DC offset. Per-probe voltage calibration lives in Settings ("Scope" group): a manual trim, or apply a steady 5.00V DC reference and turn the knob once on the Auto-cal item - plus a 3-waveform (sine/triangle/rectangle) signal generator
...
- Joystick channel linking: one row of on/off channel buttons per control (Steer, Throttle) in Settings - any channel you switch on drives together with that control, each remapped to its own Min/Center/Max so it stays symmetric even with different calibration, saved immediately on every tap. Only applies in Joystick Mode - dragging the Steer/Throttle channel's own slider in Manual Mode moves just that channel, not its linked partners
