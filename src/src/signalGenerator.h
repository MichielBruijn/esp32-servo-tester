...
void stopAll()
{
  ledcDetachPin(SIGNAL_GENERATOR_PIN);
  i2s_driver_uninstall((i2s_port_t)0);
  dac_output_disable(DAC_CHANNEL_1);
  dac_i2s_disable();
  pinMode(SIGNAL_GENERATOR_PIN, OUTPUT); // Required in order to make rectangle working after sinus mode
  initDone = false;
...
// SIGNAL_GENERATOR_PIN als Ausgang zuweisen
void startRectangle()
{
  ledcAttachPin(SIGNAL_GENERATOR_PIN, 2);
  initDone = true;
...
void startTriangle()
{
  // The I2S built-in DAC mode drives both DAC channels (GPIO25 and GPIO26) with the same buffer by
  // default. GPIO26 is the joystick click button now, so explicitly restrict output to the right
  // I2S channel only, which maps to DAC channel 1 / GPIO25 (SIGNAL_GENERATOR_PIN).
...
double triangleSetFrequency(double frequency, uint8_t ratio)
{
  ...
  i2s_set_dac_mode(I2S_DAC_CHANNEL_RIGHT_EN); // Re-apply: reinstalling the driver resets this to both channels
  i2s_set_pin((i2s_port_t)0, NULL); // I2S wird mit dem DAC genutzt
...
void startSinus()
{
  // Ausgang für SIGNAL_GENERATOR_PIN freigeben
  dac_output_enable(DAC_CHANNEL_1);
  // Sinusgenerator aktivieren
  SET_PERI_REG_MASK(SENS_SAR_DAC_CTRL1_REG, SENS_SW_TONE_EN);
  // Ausgabe auf Kanal 1 starten (DAC_CHANNEL_1 / GPIO25 - CW_EN2/INV2 would be channel 2 / GPIO26)
...
{
  controlGenerator();
  Serial.println("Use Pin 25 / Benutze Pin 25");
