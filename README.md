...
- **Menu reorganized**: the top-level list is now Manual Mode (was Servo Tester), Sweep Mode (was Automatic Mode), Expert Functions, Joystick Mode (mobile), Info, Settings. Expert Functions is a new nested submenu (its own turn-to-browse, press-to-enter list) grouping the less commonly used, more technical screens: Read PWM Impulse, Read PPM Multiswitch, Read SBUS, Read IBUS, Oscilloscope and Signal Generator - keeping the top-level list short while still making all of them reachable.
...
- **Firmware update via GitHub Releases**: in WiFi Station mode with internet access, the device periodically checks `api.github.com` for this repo's latest release and compares its tag against the running `codeVersion`. If newer, an "Update available" notice appears on the OLED Info page (firmware page, short press installs) and on every web page (banner + Info page, "Install Update" button). Installing streams the release's `firmware.bin` asset straight into the OTA partition via ESP32's `Update` library and restarts on success; the running firmware is left untouched if the download or write fails. Uses `WiFiClientSecure::setInsecure()` (no certificate pinning) since the ESP32 has no practical way to keep a GitHub TLS cert fresh - a deliberate trade-off for a hobby project checking its own public repo. Adds ~13% flash usage (mbedTLS/HTTPClient) - still comfortably inside the OTA partition, but with less headroom for future growth.
